                    ...
                    if (step + 1) % 1000 == 0:
                        ...
                        for step, batch in enumerate(dev_dataloader):
                            ...
                            if turn == 0:
                                ...
                                all_input_ids +=  input_ids.cpu().detach().numpy().tolist()
                                sample_ids +=  sid
                                compute_jointscore(start_scores, end_scores, gen_scores, pred_ops, ans_vocab, slot_mask)
                        ...
                        print("\n")
                        model.train()
            saveOperationLogits(model, device = device, dataset = dev_dataloader, save_path = args.output_dir, turn = turn)
        else:
